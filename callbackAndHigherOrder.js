...
// CODE HERE 
const first = (arrNames, cb)=> cb(arrNames[0])
...
// CODE HERE
const last = (arrNames, cb)=> cb(arrNames[6])
...
const contains = (arrNames, name, cb) => {
  return cb(arrNames.includes(name))
}
...
// CODE HERE
const uniq =(arrNames, cb)=>{
  for (let i=0; i<arrNames.legnth;i++){
    for (let x=i+1;x< arrNames.legnth;x++){
      if (arrNames[i]=== arrNames[x]){
        arrNames.splice(x,1)
    x--
      }
    }
